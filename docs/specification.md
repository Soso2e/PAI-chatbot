...
LLM（デフォルト: LM Studio の OpenAI 互換 API）を核とし、**Discord / Slack / HTTP API** の複数インターフェースから同一のモデル・DBを参照して会話できるチャットボット。
...
│  └─ RAG Manager（ChromaDB + Embeddings）             │
│       │                                              │
...
|---|---|---|
| `ollama` | Ollama | `{base_url}/api/chat` |
| `openai` | OpenAI API / 互換API（LM Studio含む） | `{base_url}/v1/chat/completions` |
...
3. Context Builder がプロンプトを組み立て
   ...
   d. RAG有効時は ChromaDB から関連チャンクを検索して system message に追加
...
長期記憶の検索はベクトル検索ではなく、クエリをスペース分割したキーワードマッチ（2文字以上のトークン）でヒット数をスコアリングする。RAG の文書検索とは別系統。
...
| LLM Client（Ollama） | ✅ 実装済み | 互換維持 |
| LLM Client（OpenAI互換） | ✅ 実装済み | LM Studio 前提 |
| DB Registry（Discordサーバー↔DB紐付け・パスワード管理） | ✅ 実装済み | |
| 長期記憶の自動キャプチャ（LLM抽出） | ✅ 実装済み | |
| 長期記憶のキーワード検索 | ✅ 実装済み | ベクトル検索ではなくキーワードマッチ |
| RAG（ChromaDB によるベクトル検索） | ✅ 実装済み | LM Studio の `/v1/embeddings` または Ollama `/api/embed` を利用 |
| `!history save` | ❌ 未実装 | |
