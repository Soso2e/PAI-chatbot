...
LLM（設定可能）を核とし、**Discord / HTTP（Slack等）** の複数インターフェースから同一のモデル・DBを参照して会話できるチャットボット。
...
| provider値 | 接続先 | 備考 |
|---|---|---|
| `ollama` | Ollama (`/api/chat`) | ローカル推奨 |
| `openai` | OpenAI API / 互換API | `base_url`で向き先変更可 |
| `custom` | 任意エンドポイント | OpenAI互換形式 |
...
`config/app.json` の `http.api_key` が空文字の場合は認証なし。
設定されている場合はリクエストヘッダー `X-API-Key` で検証。
...
HTTP_API_KEY=your_api_key_if_needed
```
...
| # | 項目 | 現在の想定 |
|---|---|---|
| 1 | LLM API形式 | Ollama優先、OpenAI互換も対応 |
| 2 | Slack連携方式 | HTTPエンドポイントを叩く形（Bot化は後日） |
| 3 | HTTP認証 | APIキーオプション（デフォルトなし） |
